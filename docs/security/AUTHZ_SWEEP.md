...
| `POST /feed/:id/dismiss` | FeedCard | `feed_cards.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /feed/:id/snooze` | FeedCard | `feed_cards.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /feed/:id/remind-later` | FeedCard | `feed_cards.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /feed/:id/refresh` | FeedCard | `feed_cards.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
...
| `POST /functions/feed/:id/dismiss` | FeedCard | `feed_cards.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /functions/feed/:id/snooze` | FeedCard | `feed_cards.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /functions/feed/:id/refresh` | FeedCard | `feed_cards.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /threads` | ItemThread | parent `brain_items.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `GET /threads/:id` | ItemThread | parent `brain_items.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `GET /threads/by-target` | ItemThread list | parent `brain_items.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /messages` | ThreadMessage | parent item owner | Yes | Covered via query route, partial direct | No | Yes | Needs direct message denial test |
| `GET /threads/:id/messages` | ThreadMessage list | parent item owner | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /ai/brain-items/:id/summarize` | ItemArtifact | parent `brain_items.userId` | Yes | Partial | No | Yes | Needs alias denial test |
...
| `POST /ai/convert` | Task decision | source item owner when provided | Yes | Partial | No | Yes | Needs alias denial test |
...
| `PATCH /tasks/:id` | Task | `tasks.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /tasks/:id/start` | Session via Task | `tasks.userId` | Yes | Partial | No | Yes | Needs route-specific denial test |
| `GET /sessions` | Session list | `sessions.userId` | Yes | Partial | Query userId ignored | Yes | Needs spoof query assertion |
| `POST /sessions/:id/pause` | Session | parent task owner | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /sessions/:id/finish` | Session | parent task owner | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /explore/connections/preview` | Explore source items | all source item owners | Yes | Partial | No | Yes | Needs route-specific denial test |
| `POST /explore/connections/save` | Connection artifact/feed card | all source item owners | Yes | Partial | No | Yes | Needs route-specific denial test |
...
1. Add route-specific feed action denial tests for both `/feed/*` and `/functions/feed/*`.
2. Add direct thread/message denial tests.
3. Add task/session mutation denial tests.
4. Add Explore preview/save denial tests.
5. Add AI alias denial tests.
6. Keep `/events` blocked until a scoped derived event API is designed.
7. Mark attachments production-deferred or implement full lifecycle tests before launch.
