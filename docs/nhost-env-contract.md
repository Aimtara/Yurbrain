...
- `nhost/nhost.toml` declares Hasura, Postgres 15, required Postgres resources, Auth, Storage, and an explicit Observability object for Nhost cloud deploys. The resources/observability values use inline object keys (`storage.capacity`, `compute.cpu`, `grafana.enabled`) to match current Nhost schema validation.
