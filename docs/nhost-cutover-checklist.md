...
- [x] Nhost initialization config checked in at `nhost/nhost.toml` (Hasura/Postgres/auth/storage/observability toggles) for current Nhost cloud builders; `nhost/config.yaml` is a minimal Hasura v3 metadata pointer.
