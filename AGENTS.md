...
- **Nhost**: Auth + storage via Nhost. Cloud project configuration is dashboard/default-managed; the repo intentionally keeps only Hasura v3 metadata config (`nhost/config.yaml`) plus `nhost/metadata/` to avoid overriding Nhost-managed infrastructure. Config helpers live in `packages/nhost/`, web/mobile clients in `apps/*/src/nhost/`, server admin in `apps/api/src/services/nhost/`.
