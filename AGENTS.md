...
- **Nhost**: Auth + storage via Nhost (`nhost/nhost.toml` plus Hasura v3 `nhost/config.yaml`). Config helpers in `packages/nhost/`, web/mobile clients in `apps/*/src/nhost/`, server admin in `apps/api/src/services/nhost/`.
